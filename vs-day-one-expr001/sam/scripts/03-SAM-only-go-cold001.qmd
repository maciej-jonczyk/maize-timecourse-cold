---
title: "GO analysis for expression changes in SAM in cold (expression change p-val<0.01)"
format: html
date: "2025-07-14"
...
Necessary test has been done before in [02-SAM-only-loop-regr-cold.qmd](../../../vs-day-one/sam/scripts/02-SAM-only-loop-regr-cold.qmd), here only filtering is required.

```{r load}
#| message: false
#| output: false
# files filtered at 0.05 level
load("../../../vs-day-one/sam/rdata-saved/sigs_SAM1.RDa")
load("../../../vs-day-one/sam/rdata-saved/sigs_all_SAM1.RDa")
```
...
save(list = ls(pattern = "^[as].*1$"), file = "../rdata-saved/sigs_SAM1-001.RDa")
...
save(list = ls(pattern = "^[as].*sig001$"), file = "../rdata-saved/sigs_all_SAM1-001.RDa")
...
# list of names
original_names <- grep("^d.*sam.*1$", ls(), value = TRUE)
...
rm(list=ls(pattern = "^d.*sam.*1$")) # remove intermediate objects
```
...
original_names <- grep("^gv.*sam.*1$", ls(), value = TRUE)
...
rm(list=ls(pattern = "^gv.*sam.*1$")) # remove intermediate objects

```

```{r plots lapply, cache=TRUE}
#| warning: false
original_names <- grep("^pwf.*sam.*1$", ls(), value = TRUE)
...
rm(list=ls(pattern = "^pwf.*sam.*1$")) # remove intermediate objects
...
invisible(
  lapply(original_names, function(name) {
    ...
    filename <- paste0("../results/go/", name, ".txt")
...
save(list = ls(pattern = "^gs."), file = "../rdata-saved/goseq-res_SAM1-001.RDa")
save(list = ls(pattern = "^bh."), file = "../rdata-saved/goseq-bh-res_SAM1-001.RDa")
