---
title: "Likelihood Ratio Test for each pair of lines (*abandoned*)"
format: html
date: "2026-03-17"
...
To check if there are time-dependent changes between lines we test the timepoint:line interaction test with [LRT test](https://master.bioconductor.org/packages/release/workflows/vignettes/rnaseqGene/inst/doc/rnaseqGene.html#time-course-experiments).
For this purpose the full dataset for SAM must be splitted to contain only two compared lines
...
dds=DESeqDataSetFromMatrix(countData=cts.sam, colData=coldata.sam, rowRanges=gzakresy, design= ~ line + timepoint + line:timepoint)
...
### Extract counts and coldata and split by line-pairs

```{r split}
coldata.f = as.data.frame(colData(dds.f))
cts.f = as.data.frame(assay(dds.f))
```
...
# genes forom the following object will be used as background in GO enrichemnt analysis
...
# saving
save(list = to_save, file="../rdata-saved/sigs_all_SAM1.RDa")
```
