...
:::{.callout-note}
This script is a greatly improved version of sum of the [10-NAMzUP-sam-networks-cytoscape-loop-ctrl.qmd](../../../project1-ctrl-stat/vs-day-one/sam/scripts/10-NAMzUP-sam-networks-cytoscape-loop-ctrl.qmd) and [10-NAMzUP-sam-rel3-networks-cytoscape-ctrl.qmd](../../../project1-ctrl-stat/vs-day-one/sam/scripts/10-NAMzUP-sam-rel3-networks-cytoscape-ctrl.qmd). Joining part was changed from `bash` to `R`.
:::
...
`cl13_m1.25` contains data for clusters.
...
cl_vec   <- cl13_m1.25$cluster               # wektor długości 11936, nazwy = geny
mem_mat  <- cl13_m1.25$membership            # macierz 11936 × 10
...
mem_cols <- grep("^mem_", names(cl13_m1.25_mem07), value = TRUE)

# 2 Obliczamy maksimum w każdym wierszu (ignorujemy NA)
max_mem <- apply(cl13_m1.25_mem07[mem_cols], 1, max, na.rm = TRUE)

# 3 Tworzymy nową ramkę z wybranymi kolumnami + maksymalną wartością
cl13_m1.25_maxmem <- data.frame(
  gene        = cl13_m1.25_mem07$gene,
  cluster     = cl13_m1.25_mem07$cluster,
  max_mem     = max_mem,
...
cl13_m1.25_sort <- cl13_m1.25_maxmem[order(cl13_m1.25_maxmem$gene), ]

write.table(cl13_m1.25_sort, file="../results/network/cl13_m1.25_sort", sep="\t", quote=FALSE, row.names=FALSE)
...
# join and sort by UP
join -1 2 -2 1 -t $'\t' ../../../../input-data/networks-up/best-nam-string-ids cl13_m1.25_sort | sort -k2,2 -t $'\t'> c13_UP_srt
...
# exclude them
grep -Fwvf xduplicated c13_UP_srt > x2
mv x2 c13_UP_srt
rm x*
```
...
cut -f2 -d $'\t' c13_UP_srt | sort -u > c13_ids_up

# extract interactions for selected UPs from full network
zgrep -Fwf c13_ids_up ../../../../input-data/networks-up/full-network.gz > c13_UP_net_selected
```
...
xUP1 = read.table("../results/network/c13_UP_net_selected", sep=' ', header = FALSE)
...
x2NAM1 = left_join(xUPsNAMs, cl13_m1.25_sort,
                   by = c('ensembl1' = 'gene'),
...
x3NAM = left_join(x2NAM1, cl13_m1.25_sort,
                       by = c('ensembl2' = 'gene'),
                       relationship = "many-to-one") |>
...
join -j1 -t $'\t' cl13_m1.25_sort ~/x3 > ~/x4
...
join -j1 -t $'\t' -a1 -a2 -o auto cl13_m1.25_sort ~/x3 | awk -v FS='\t' -v OFS='\t' '$2=="" && $4==""' > ~/x5
# grep them from network assignment file
grep -Fwf ~/x5 cl13_m1.25_sort > ~/x2
...
grep Zm00001eb269840 cl13_m1.25_sort
