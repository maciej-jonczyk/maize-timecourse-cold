...
:::{.callout-note}
This script is a greatly improved version of sum of the [10-NAMzUP-leaf-networks-cytoscape-loop-ctrl.qmd](../../../project1-ctrl-stat/vs-day-one/leaf/scripts/10-NAMzUP-leaf-networks-cytoscape-loop-ctrl.qmd) and [10-NAMzUP-leaf-rel3-networks-cytoscape-ctrl.qmd](../../../project1-ctrl-stat/vs-day-one/leaf/scripts/10-NAMzUP-leaf-rel3-networks-cytoscape-ctrl.qmd). Joining part was changed from `bash` to `R`.
:::
...
`cl15_m1.25` contains data for clusters.
...
cl_vec   <- cl15_m1.25$cluster               # wektor długości 11936, nazwy = geny
mem_mat  <- cl15_m1.25$membership            # macierz 11936 × 10
...
mem_cols <- grep("^mem_", names(cl15_m1.25_mem07), value = TRUE)

# 2 Obliczamy maksimum w każdym wierszu (ignorujemy NA)
max_mem <- apply(cl15_m1.25_mem07[mem_cols], 1, max, na.rm = TRUE)

# 3 Tworzymy nową ramkę z wybranymi kolumnami + maksymalną wartością
cl15_m1.25_maxmem <- data.frame(
  gene        = cl15_m1.25_mem07$gene,
  cluster     = cl15_m1.25_mem07$cluster,
  max_mem     = max_mem,
...
cl15_m1.25_sort <- cl15_m1.25_maxmem[order(cl15_m1.25_maxmem$gene), ]

write.table(cl15_m1.25_sort, file="../results/network/cl15_m1.25_sort", sep="\t", quote=FALSE, row.names=FALSE)
...
# join and sort by UP
join -1 2 -2 1 -t $'\t' ../../../../input-data/networks-up/best-nam-string-ids cl15_m1.25_sort | sort -k2,2 -t $'\t'> c15_UP_srt
...
# 10133 # some UPs map to multiple NAMs

# Check
cut -f2 -d $'\t' c15_UP_srt | sort | uniq -d > x
grep -Fwf x c15_UP_srt > x2
...
# 46 NAMs
wc -l xduplicated
# 19 proteins corresponding to duplicated NAMs are in different clusters

# exclude them
grep -Fwvf xduplicated c15_UP_srt > x2
mv x2 c15_UP_srt
rm x*
```
...
zgrep -Fwf c15_ids_up ../../../../input-data/networks-up/full-network.gz > c15_UP_net_selected
```
...
xUP1 = read.table("../results/network/c15_UP_net_selected", sep=' ', header = FALSE)
...
NAM_UP_anno = read.table("../../../input-data/go-uniprot/NAM-UP-anno", sep='\t', header = FALSE, quote = "")
...
x2NAM1 = left_join(xUPsNAMs, cl15_m1.25_sort,
                   by = c('ensembl1' = 'gene'),
...
x3NAM = left_join(x2NAM1, cl15_m1.25_sort,
                       by = c('ensembl2' = 'gene'),
                       relationship = "many-to-one") |>
...
join -j1 -t $'\t' cl15_m1.25_sort ~/x3 > ~/x4
...
join -j1 -t $'\t' -a1 -a2 -o auto cl15_m1.25_sort ~/x3 | awk -v FS='\t' -v OFS='\t' '$2=="" && $4==""' > ~/x5
# grep them from network assignment file
grep -Fwf ~/x5 cl15_m1.25_sort > ~/x2
...
# NAM - UP assignment
grep Zm00001eb314790 ../../../../input-data/go-uniprot/BEST_UPs
grep A0A1D6LKH6 ../../../../input-data/go-uniprot/BEST_UPs

# NAM and UP description
grep Zm00001eb269840 ../../../../input-data/go-uniprot/NAM_descr_best
grep A0A1D6LKH6 ../../../../input-data/go-uniprot/UP_descr_best

## Cluster assignment
# leaf/results/network dir
grep Zm00001eb269840 cl15_m1.25_sort
...
```{bash rhythm-add}
# Include manually added clock genes

cd ../results/network/

for i in {1..15}; do
awk '
    NR==FNR {           # Czytamy pierwszy plik (rel3-genes-rhythm-add)
        keep[$1] = 1;       # budujemy tablicę asocjacyjną – klucz = wartość
       next;             # przejdź do kolejnego rekordu (nie analizujemy dalej)
    }
    $2 in keep {         # Drugi plik (klastry); sprawdzamy kolumnę 2
...
' ../../../../input-data/go-uniprot/rel3-genes-rhythm-add cluster_${i}_400.txt > x
  cat header-net-tab x > cluster_${i}_400_rel3_rhythm_add.txt;
done

## Remove files without data

files=($(ls cluster_*_400_rel3_rhythm_add.txt))
