---
title: "Mfuzz clustering of leaf data, minimum 3 significant genes from point by point comparisons, leaf."
format: html
date: "2026-03-19"
...
The DE-analysis compares each time point for each pair of maize lines.
...
```{r load}
load("../rdata-saved/contr_pair_leaf_sig005.RDa")
```
...
original_names = ls(pattern = "leaf_sig$")
...
rm(list = original_names)
```

### Join vectors, filter-out significant in less than 3 points

```{r join}
vectors <- mget(ls(pattern = "^ids.*"))

# concatenate
joined_ids <- unlist(vectors)

# how many times gene changes
licznik = table(joined_ids)

#hist(licznik, breaks = c(1 : 72))

# filter-out genes significant < 3 times
licznik_min3 <- licznik[licznik > 2]

#hist(licznik_min3, breaks = c(1 : 72))
...
# remove original objects
rm(list=c("cts", "coldata", "multiplot"))
```
...
# at least 3 leafples with a count of 10 or higher
...
```{r counts}
# select sigs
vsd.sigs = vsd[rownames(vsd) %in% ids_min3, ]

# varMetadata - description of variables
metadata=data.frame(labelDescription=c("pooled sample", "letter", "tissue", "line", "hour of pooling", "sample number", "day", "replication", "condition", "id", "reads [million]", "batch [sequencing]", "million of reads binned", "grouping variable for averaging", "chronological time point", "group", "sizeFactor"), row.names=colnames(colData(vsd.sigs)))
...
save(vsd.eset.ave.std, file="../rdata-saved/vsd.eset.ave.std.leaf_pair_min3pt.RDa")
```
...
tmpR=partcoef(vsd.random, crange=seq(7, 20, 1), mrange=seq(1.05, 1.45, 0.05))
...
```{r check random}
# RANDOM
# For random data even m = 1.05 gives no clustering

##############################
# Adjust parameters manually #
##############################

c_val <- 10 ; m_val <- 1.05 # uniform partitioning
...
#c_vals = c(8 : 15) # Dmin
c_vals = 15
m_vals = c(1.25, 1.3) #seq(1.15, 1.45, 0.05) # Dmin


## Uncomment on new session
# load("../rdata-saved/vsd.eset.ave.std.leafALLsig.RDa")
source("../../../vs-day-one/sam/scripts/mfuzz.plot2_separate_lines.R") # modified plotting function, tailored to separate lines
...
for (c_val in c_vals) {
  for (m_val in m_vals) {
    ...
    key <- paste0("cl", c_val, "_m", m_val)
    results[[key]] <- cl

# Commented plot-generating code, not needed for final clusters
    
    # #dynamic naming of file
    # filename <- paste0("cl", c_val, "_m", m_val, ".pdf")
    # 
    # # save plot
    # pdf(filename)
    ...
    # # dynamic naming of file
    # filename07 <- paste0("cl", c_val, "_m", m_val, "minmem07.pdf")
    # 
    # # save plot
    # pdf(filename07)
...
## Uncomment on new session
load("../rdata-saved/vsd.eset.ave.std.leaf_pair_min3pt.RDa")

source("../../../vs-day-one/sam/scripts/mfuzz.plot2_separate_lines.R") # modified plotting function, tailored to seprarate lines
...
for (cluster in 1:c_val) {
    ...
    mfuzz.plot2mod(vsd.eset.ave.std, single=cluster, cl = cl, mfrow = c(1,1), x11 = FALSE, centre=TRUE, centre.col="black", centre.lwd=2,
                time.labels = colnames(vsd.eset.ave.std),  cex.axis = 0.6, las = 2, ylim = c(-4,4))
    dev.off()
...
cl15_m1.25 <- results[["cl15_m1.25"]]

# this file contains only genes with mem >=0.7
genes_in_clusters <- lapply(1:ncol(cl15_m1.25$membership), function(i) {
  geny <- rownames(cl15_m1.25$membership)[cl15_m1.25$membership[, i] >= 0.7]
  return(geny)
})

# Name clusters
names(genes_in_clusters) <- paste("Klaster", 1:ncol(cl15_m1.25$membership))

save(cl15_m1.25, genes_in_clusters, file="../rdata-saved/mfuzz_cl15_m1.25_leaf.RDa")
