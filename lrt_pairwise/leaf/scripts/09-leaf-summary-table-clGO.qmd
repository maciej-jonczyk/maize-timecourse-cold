...
# goseq results
load("../rdata-saved/mfuzz_res_for09_leaf.RDa")
...
go_anno <- read.table("../../../input-data/go-uniprot/GO_descr_NAM_descr", header = FALSE, stringsAsFactors = FALSE, sep='\t', quote = "")
...
write.table(cl15_m1.25_desc, file="../results/go/leaf-cluster-membership.txt", sep="\t", quote=FALSE, row.names=FALSE)
...
```{r go-summary}
# Join everything
# Join cluster summary (cl15_m1.25_desc) with GO description and gene description
...
# save to file
write.table(cluster_go_summary, file="../results/go/leaf-cluster-go-summary.txt", sep="\t", quote=FALSE, row.names=FALSE)
```
...
echo -e 'Num_genes\tCluster' > leaf-cluster-num-genes.txt
cut -f2 -d $'\t' leaf-cluster-membership.txt | tail -n +2 | sort -n| uniq -c | sed -E 's/^ +//;s/ /\t/' >> leaf-cluster-num-genes.txt
...
# save to file
write.table(len07table, file="../results/go/leaf-cluster-num-genes-mem07.txt", sep="\t", quote=FALSE, row.names=FALSE)
```
