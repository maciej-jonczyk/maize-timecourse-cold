---
title: "GO analysis for expression changes in SAM in cold, comparisons to 2nd day of cold (expression change p-val<0.01)"
format: html
date: "2025-07-15"
...
Necessary test has been done before in [02-SAM-only-loop-regr-cold2.qmd](../../../vs-day-two/sam/scripts/02-SAM-only-loop-regr-cold2.qmd), here only filtering is required.

```{r load}
#| message: false
#| output: false
# files filtered at 0.05 level
load("../../../vs-day-two/sam/rdata-saved/sigs-SAM2.RDa")
load("../../../vs-day-two/sam/rdata-saved/sigs_all_SAM2.RDa")
```
...
save(list = ls(pattern = "^[as].*1$"), file = "../rdata-saved/sigs_SAM2-001.RDa")
...
save(list = ls(pattern = "^[as].*sig001$"), file = "../rdata-saved/sigs_all_SAM2-001.RDa")
rm(list=ls(pattern = "^[as].*sig.*")) # remove objects - not needed here
```
...
rm(list=ls(pattern = "^[as].*(min|pl)001$")) # remove original objects
...
# list of names
original_names <- grep("^d.*(min|pl)001$", ls(), value = TRUE)
...
rm(list=ls(pattern = "^d.*(min|pl)001$")) # remove intermediate objects
```
...
original_names <- grep("^gv.*(min|pl)001$", ls(), value = TRUE)
...
rm(list=ls(pattern = "^gv.*(min|pl)001$")) # remove intermediate objects

```

```{r plots lapply}
#| warning: false
original_names <- grep("^pwf.*(min|pl)001$", ls(), value = TRUE)
...
rm(list=ls(pattern = "^pwf.*(min|pl)001$")) # remove intermediate objects
...
save(list = ls(pattern = "^gs."), file = "../rdata-saved/goseq-res_SAM2-001.RDa")
save(list = ls(pattern = "^bh."), file = "../rdata-saved/goseq-bh-res_SAM2-001.RDa")
