---
title: "GO analysis for expression changes in leaf in cold, comparisons to 2nd day of cold (expression change p-val<0.01)"
format: html
date: "2025-07-15"
...
Necessary test has been done before in [02-leaf-only-loop-regr-cold2.qmd](../../../vs-day-two/leaf/scripts/02-leaf-only-loop-regr-cold2.qmd), here only filtering is required.

```{r load}
# files filtered at 0.05 level
load("../../../vs-day-two/leaf/rdata-saved/sigs-leaf2.RDa")
```
...
save(list = ls(pattern = "^[as].*1$"), file = "../rdata-saved/sigs-leaf2-001.RDa")
```

## Files for GO analysis

:::{.callout-note}
This coded depends on the prevous code [02-leaf-only-loop-regr-cold2.qmd](02-leaf-only-loop-regr-cold2.qmd) and [03-leaf-only-go-cold.qmd](03-leaf-only-go-cold.qmd). Some files and objects has been done in those files.
:::
...
save(list = ls(pattern = "^gs."), file = "../rdata-saved/goseq-res-leaf2-001.RDa")
save(list = ls(pattern = "^bh."), file = "../rdata-saved/goseq-bh-res-leaf2-001.RDa")
