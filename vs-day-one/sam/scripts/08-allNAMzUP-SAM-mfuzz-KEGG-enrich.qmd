---
title: "KEGG pathway enrichment analysis for expression changes in SAM in cold - ALL significant genes"
format: html
date: "2026-01-07"
...
expr.paths=read.table("../../../input-data/kegg-up/SAM_path.pop", header = FALSE, stringsAsFactors = FALSE)
...
Load the data from the mfuzz analysis for given number of clusters and fuzziness (computed in [07-SAM-only-mfuzzALLsig.qmd](07-SAM-only-mfuzzALLsig.qmd))
...
```{r save}
SAM_kegg_summary <- do.call(rbind, lapply(names(result_clean), function(n) {
  df <- result_clean[[n]]
  df$cluster <- sub("Klaster ", "", n)  # Wyodrębnij numer
  return(df)
}))

write.table(SAM_kegg_summary, file="../results/kegg/samALLsig_kegg_enrich_summary.txt", sep="\t", row.names=FALSE, col.names=TRUE, quote=FALSE)
```
