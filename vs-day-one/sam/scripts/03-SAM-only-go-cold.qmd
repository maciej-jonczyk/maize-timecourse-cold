---
title: "GO analysis for expression changes in SAM in cold"
format: html
date: "2025-07-09"
...
Load the data from the significance test `02-SAM-only-loop-regr-cold.qmd ` (significant genes splitted by direction of change)
...
rm(list=ls(pattern = "^[as].*sam_(min|pl)$")) # remove original objects
...
# list of names
original_names <- grep("^d.*sam_(min|pl)$", ls(), value = TRUE)
...
rm(list=ls(pattern = "^d.*sam_(min|pl)$")) # remove intermediate objects
```
...
original_names <- grep("^gv.*sam_(min|pl)$", ls(), value = TRUE)
...
rm(list=ls(pattern = "^gv.*sam_(min|pl)$")) # remove intermediate objects

```

```{r plots lapply, cache=TRUE}
#| warning: false
original_names <- grep("^pwf.*sam_(min|pl)$", ls(), value = TRUE)
...
rm(list=ls(pattern = "^pwf.*sam_(min|pl)$")) # remove intermediate objects

```

<!-- Remove empty vectors -->
<!-- ```{r remove empty} -->
<!-- # For comparisons without significant GOs there are empty objects. -->
<!-- # They are removed -->
...
<!-- ## The pattern MUST be determined manually beforehand (based on 'empty_vars') -->
...
save(list = ls(pattern = "^gs."), file = "../rdata-saved/goseq-res_SAM1.RDa")
save(list = ls(pattern = "^bh."), file = "../rdata-saved/goseq-bh-res_SAM1.RDa")
