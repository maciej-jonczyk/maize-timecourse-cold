...
cl_vec   <- c13_m1.35$cluster               # wektor długości 11936, nazwy = geny
mem_mat  <- c13_m1.35$membership            # macierz 11936 × 10
...
# join and sort by UP
join -1 2 -2 1 -t $'\t' ../../../../input-data/networks-up/best-nam-string-ids c13_m1.35_sort | sort -k2,2 -t $'\t'> c13_UP_srt
...
# 3927 # some UPs map to multiple NAMs

# Check
cut -f2 -d $'\t' c13_UP_srt | sort | uniq -d > x
grep -Fwf x c13_UP_srt
# Zm00001eb173710 B4FML8  8       0.922419768588153
# Zm00001eb173730 B4FML8  8       0.705660698699791
# Zm00001eb296560 B4G1P4  5       0.728658534402088
# Zm00001eb296580 B4G1P4  1       0.860659460036542
# Zm00001eb019230 B6SGI4  8       0.790375096952795
# Zm00001eb229690 B6SGI4  8       0.711050526774375
# Zm00001eb185510 B6T969  1       0.862093623477879
# Zm00001eb185540 B6T969  10      0.877677319194626
# Zm00001eb028980 B6TGT0  8       0.816783789906092
# Zm00001eb136620 B6TGT0  8       0.726828882765232
# Zm00001eb202910 Q9ZQX9  8       0.765588981641404
# Zm00001eb241090 Q9ZQX9  6       0.830930901211403

# So for B4G1P4, B6T969 and Q9ZQX9 duplicated NAMs are in different clusters

# exclude them
grep -Fwv B4G1P4 c13_UP_srt | grep -Fwv B6T969 | grep -Fwv Q9ZQX9 > x2
mv x2 c13_UP_srt
```
...
grep Zm00001eb269840 c13_m1.35_sort
...
```{r split-GO}
# If not done previously or if other categories are needed
#
...
```{bash go check}
# If new categories were extracted
...
# cd ../../vs-day-one/leaf/results/network/
...
```{bash photosynthesis}
# select genes annotated with photosynthesis
...
' ../../../../input-data/go-uniprot/genes-photosyn cluster_${i}_400.txt > x
