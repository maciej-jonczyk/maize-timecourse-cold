...
Loading object created in `cleanup2` chunk is sufficient
:::
...
c_val <- 8 ; m_val <- 1 # produces uniform partitioning
...
c9_m1.3 <- results[["c9_m1.3"]]


genes_in_clusters <- lapply(1:ncol(c9_m1.3$membership), function(i) {
  geny <- rownames(c9_m1.3$membership)[c9_m1.3$membership[, i] >= 0.7]
  return(geny)
})

# Name clusters
names(genes_in_clusters) <- paste("Klaster", 1:ncol(c9_m1.3$membership))

save(c9_m1.3, genes_in_clusters, file="../rdata-saved/mfuzz_c9_m1.3_leafALLsig.RDa")
